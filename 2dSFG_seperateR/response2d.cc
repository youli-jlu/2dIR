  ...
  parameter(parameterin),trans1(transin1),trans2(transin2) 
{
  ...
  R1_r=new complex<double>[ntotal];
  R1_nr=new complex<double>[ntotal];
  R2_r=new complex<double>[ntotal];
  R2_nr=new complex<double>[ntotal];
}
response2d::~response2d() {};
void response2d::initial() {};

complex<double> & response2d::r1(int t1,int t3)
{
  int index=t1*tmax+t3;
  return R1_r[index];
}
complex<double> & response2d::nr1(int t1,int t3)
{
  int index=t1*tmax+t3;
  return R1_nr[index];
}
complex<double> & response2d::r2(int t1,int t3)
{
  int index=t1*tmax+t3;
  return R2_r[index];
}
complex<double> & response2d::nr2(int t1,int t3)
{
  int index=t1*tmax+t3;
  return R2_nr[index];
}
...
void response2d::printr()
{
  ...
  for (int i=0;i<tmax;i++)
  {
    ...
    {
      outfile<<i*parameter.dt<< "  " << j*parameter.dt<< " "  
        << r1(i,j).real()  <<  "  "  << r1(i,j).imag()  << " "
        << nr1(i,j).real()  <<  "  "  << nr1(i,j).imag() << " "
        << r2(i,j).real()  <<  "  "  << r2(i,j).imag()  << " "
        << nr2(i,j).real()  <<  "  "  << nr2(i,j).imag() 
        << endl; 
    }
  }
...
//  outfile << "t1" << "  " << "t3" << "  "<< "re"  <<  "  "  << "im" << endl; 
//  for (int i=0;i<tmax;i++)
//  {
//    for (int j=0;j<tmax;j++)
//    {
//      outfile<<i*parameter.dt<< "  " << j*parameter.dt<< " "  << r(i,j).real()  <<  "  "  << r(i,j).imag() << endl; 
//    }
//  }
//  outfile.close();
//}
...
//    for (int j=0;j<tmax;j++)
//    {
//      outfile<<i*parameter.dt<< "  " << j*parameter.dt<< " "  << nr(i,j).real()  <<  "  "  << nr(i,j).imag() << endl; 
//    }
//  }
//}
...
{
  if (parameter.nzp>0){
    zero_padding1d(R1_r,parameter.nzp);
    zero_padding1d(R1_nr,parameter.nzp);
    zero_padding1d(R2_r,parameter.nzp);
    zero_padding1d(R2_nr,parameter.nzp);
    tmax+=parameter.nzp;
...
void condon2d::calculation()
{
  ...
  for(int k=0;k<parameter.np;k++)
  {
    ...
    for(int t1=0;t1<tmax;t1++)
    {
      ...
      {
        delta=domega*t3;
        r1(t1,t3) += (2.0*td10*exp(double( f-g10)*1i)*gamma_SE(t1,t3))/double(parameter.np);
        nr1(t1,t3)+= (2.0*td10*exp(double(-f-g10)*1i)*gamma_SE(t1,t3))/double(parameter.np);
        r2(t1,t3) += (- td21*exp(double( f-g21+delta)*1i)*gamma_TA(t1,t3))/double(parameter.np);
        nr2(t1,t3)+= (- td21*exp(double(-f-g21+delta)*1i)*gamma_TA(t1,t3))/double(parameter.np);
...
void non_condon2d::calculation()
{
  ...
  for(int k=0;k<parameter.np;k++)
  {
    ...
    for(int t1=0;t1<tmax;t1++)
    {
      ...
      {
        delta=domega*t3;
        r1(t1,t3) += (2.0*td10_t(t1,t3,tk)*exp(double( f-g10)*1i)*gamma_SE(t1,t3))/double(parameter.np);
        nr1(t1,t3)+= (2.0*td10_t(t1,t3,tk)*exp(double(-f-g10)*1i)*gamma_SE(t1,t3))/double(parameter.np);
        r2(t1,t3) += (- td21_t(t1,t3,tk)*exp(double( f-g21+delta)*1i)*gamma_TA(t1,t3))/double(parameter.np);
        nr2(t1,t3)+= (- td21_t(t1,t3,tk)*exp(double(-f-g21+delta)*1i)*gamma_TA(t1,t3))/double(parameter.np);
        g10+=trans1.freq[tk+t1+parameter.t2+t3];
        g21+=trans2.freq[tk+t1+parameter.t2+t3];
      }
      f+=trans1.freq[tk+t1];
...
//cumulant2d method
cumulant2d::cumulant2d(para parameterin,transition transin1,transition transin2)
  :response2d(parameterin,transin1,transin2) {} ;

double cumulant2d::gamma_TA(int t1,int t3) { 
  return exp( -(gamma1+gamma2)*t3/2.0 -gamma1*parameter.t2 - gamma1*t1/2.0 );}
double cumulant2d::gamma_SE(int t1,int t3) { 
  return exp( -(gamma1+gamma2)*t3/2.0 -gamma1*parameter.t2 - gamma1*t1/2.0 );}

double* cumulant2d::correlation_function(string filename,transition transin1,transition transin2)
{
  int tk;
  double *g;
...
void cumulant2d::calculation()
{
  ...
  for (int t1=0;t1<tmax;t1++)
  {
    for (int t3=0;t3<tmax;t3++)
    {
      ...
      r1(t1,t3) =(2.0*td10*exp(G1)*gamma_SE(t1,t3) )/double(parameter.np);
      nr1(t1,t3)=(2.0*td10*exp(G3)*gamma_SE(t1,t3) )/double(parameter.np);
      r2(t1,t3) =- td21*exp(G2)*exp(1i*delta)*gamma_TA(t1,t3)/double(parameter.np);
      nr2(t1,t3)=- td21*exp(G4)*exp(1i*delta)*gamma_TA(t1,t3)/double(parameter.np);
    }
  }
  time[1]=clock();
  cout << "total time for response function:"
