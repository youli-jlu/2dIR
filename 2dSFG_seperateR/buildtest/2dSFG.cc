...
void spectra2d::calculation()
{
  ...
  readtp(parameter.tpfile1,trans1);
  readtp(parameter.tpfile2,trans2);
...
// read transition dipole in given direction
// e.g. td=[u_k(t),u_l(t),u_m(t)]
void spectra2d::readtd(string filename,transition &transi)
{
  ...
  if (parameter.responseflag=="non-condon")
  {
    while(tdfile.peek() != EOF ){
      ...
      transi.td_k.push_back(td[0]); transi.td_l.push_back(td[1]); transi.td_m.push_back(td[2]);
...
// read transition polarizability in given direction
// e.g. tp=a_ij(t)
void spectra2d::readtp(string filename,transition &transi)
{
  ifstream tpfile(filename,ios::in);
  string oneline;
  int nd=0;
  double tp[1];
  if (!tpfile) {
    cerr << "open tpfile fail\n" ;
    cerr << "file:"<< filename <<"doesn't exist\n" << endl;
    exit(0);}
  getline(tpfile,oneline); // remove fisrt line
  if (parameter.responseflag=="non-condon")
  {
    while(tpfile.peek() != EOF ){
      nd++;
      tpfile >> tp[0] ;
      transi.tp_ij.push_back(tp[0]); 
      getline(tpfile,oneline); // remove other character
      for (int i=0;i<parameter.rtgap-1;i++) getline(tpfile,oneline); // skip oneline
    }
    if (nd<parameter.n) {
      cerr << "too less transition polarizability\n" <<endl ;
      exit(0);
    }
  }
  else
