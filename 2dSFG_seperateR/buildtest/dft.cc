...
dft2d::dft2d(para parameterin,complex<double>* rin,complex<double>* nrin):
 // dft(parameterin),r1(rin),nr1(nrin),r2(rin),nr2(nrin) {};
...
void dft2d::spectra(string spectrafile)
{
  cout << " *** spectra information *** " << endl;
  ...
  ofstream spectra(spectrafile,ios::out);
  ...
  spectra << "w1    w3     I_r I_nr I_total"  << endl;
  int indexr,indexnr;
...
void dft1d::spectra(string spectrafile)
{
  ...
  cout << "print spectra" << endl;
  spectra << "w1    rephasing   non-rephasing  total:" << tmax << endl;
  spectra << "w1    I"  << endl;
  for (int w1=nmin;w1<nmax;w1++) 
  {
      spectra<< setprecision(10)  << parameter.momega1+ parameter.dw*(w1-tmax/2) << "  " ;
