...
void spectra1d::calculation()
{
  ...
  if (parameter.responseflag=="condon") {
    ...
    R= new cumulant1d(parameter,trans1); std::cout << " Doing 1d cumulant calculation "<< std::endl;}
  else {
...
// read transition property (e.g.dipole) in given direction
void spectra1d::readtp()
{
  ifstream tpfile(parameter.tpfile1,ios::in);
  string oneline;
  int nd=0;
  double tp[1];
  if (!tpfile) {
    cerr << "open tpfile fail\n" ;
    cerr << "file:"<<parameter.tpfile1 <<"doesn't exist\n" << endl;
      exit(0);}
