  ...
  for (int i=0;i<tmax;i++)
  {
    for (int j=0;j<tmax;j++)
    {
      outfile<<i*parameter.dt<< "  " << j*parameter.dt<< " "  
        << r(i,j).real()  <<  "  "  << r(i,j).imag()  << " "
        << nr(i,j).real()  <<  "  "  << nr(i,j).imag() 
        << endl; 
    }
  }
...
void non_condon2d::calculation()
{
  ...
  for(int k=0;k<parameter.np;k++)
  {
    ...
    for(int t1=0;t1<tmax;t1++)
    {
      ...
      {
        delta=domega*t3;
        r(t1,t3) += (2.0*td10_t(t1,t3,tk)*exp(double( f-g10)*1i)*gamma_SE(t1,t3) 
            - td21_t(t1,t3,tk)*exp(double( f-g21+delta)*1i)*gamma_TA(t1,t3))/double(parameter.np);
        nr(t1,t3)+= (2.0*td10_t(t1,t3,tk)*exp(double(-f-g10)*1i)*gamma_SE(t1,t3) 
            - td21_t(t1,t3,tk)*exp(double(-f-g21+delta)*1i)*gamma_TA(t1,t3))/double(parameter.np);
        g10+=trans1.freq[tk+t1+parameter.t2+t3];
        g21+=trans2.freq[tk+t1+parameter.t2+t3];
      }
