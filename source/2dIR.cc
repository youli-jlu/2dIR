...
void spectra2d::calculation()
{
  ...
  if (parameter.responseflag=="condon") {
    ...
    R= new cummulant2d(parameter,trans1,trans2); std::cout << " Doing 2d cummulant calculation "<< std::endl;}
  else {
  ...
  R->printr("2drephasing");
  R->printnr("2dnon-rephasing");
  ...
  dft2d dft1(parameter,R->R_r,R->R_nr);
  dft1.spectra();
