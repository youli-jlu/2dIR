...
void response1d::printr(string filename)
{
  ...
  {
    outfile <<i<< "  "  << r(i).real()  <<  "  "  << r(i).imag() << endl; 
  }
  outfile.close();
}
...
void condon1d::calculation()
{
  ...
  for(int k=0;k<parameter.np;k++)
  {
    ...
    {
      r(t1) += (td10*exp(f*1i)*gamma(t1))/double(parameter.np);
      f+=trans1.freq[tk+t1];
    }
  }
...
double non_condon1d::td10(int t1,int tk) { return trans1.td[tk]*trans1.td[tk+t1];}
